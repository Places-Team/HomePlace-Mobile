...
class DeviceCredentialStore(context: Context) {
    ...
    fun publicKey(serverId: String): String {
        val alias = identityAliasFor(serverId)
        val existing = keyStore.getCertificate(alias)?.publicKey
        val key = existing ?: KeyPairGenerator.getInstance(KeyProperties.KEY_ALGORITHM_EC, "AndroidKeyStore").run {
            initialize(
    ...
    private fun identityAliasFor(serverId: String): String = "homeplace.identity.${serverId.hashCode().toUInt()}"
