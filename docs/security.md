...
Android may optionally discover incoming clipboard, text, URL and file offers during the same periodic background check. The background worker validates the server identity and bounded event metadata, displays a private notification without embedding the shared value or filename, and deliberately leaves the event unacknowledged. Accept and Decline actions are recorded by server profile and event ID; processing still revalidates the profile and event before any acknowledgement. The optional seamless setting accepts only files whose event carries the server-derived `sameAccount` marker, and only while the application is open. It never applies to household devices, URLs, text, or clipboard. Notification deduplication stores event identifiers only. Multiple offers are queued independently and remain isolated to the authenticated profile and server-enforced account or household scope.
