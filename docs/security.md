...
Android sharing is deliberately consent-driven. Selecting HomePlace in the system Share Sheet only imports a preview into the application. A multi-file batch is limited to ten items and 500 MB total before it enters private application cache. The user must choose one named device and confirm the batch; each item is still a separately scoped 30-minute server offer. The server always allows targets owned by the same `userId`; a device owned by another household user appears only when an administrator has explicitly enabled household sharing for that receiving device. Household ownership is repeated in the confirmation. Unknown and unauthorized targets receive the same generic unavailable response.

Text is limited to 8,000 characters. Links are limited to HTTP(S), may not contain embedded credentials, and are never opened automatically. Files are limited to 500 MB, streamed instead of being held in memory, encrypted at rest by the server with a random per-transfer AES-256-GCM key, and offered to exactly one target for 30 minutes. This covers at least one normally delayed Android background check while keeping payloads short-lived; clipboard offers still expire after five minutes. The client checks the response metadata, exact byte count, and advertised SHA-256 digest before asking the Android host to copy a host-created private cache file into `Downloads/HomePlace`. The Android host returns only a scoped `content://` handle for the completed download; opening it grants temporary read access to the user-selected viewer. The offer is acknowledged and removed only after MediaStore confirms the save. Cancellation or any mismatch deletes the temporary client file and leaves the offer available for an explicit retry until expiry. Content is not included in logs or notification text. Declined, accepted, and expired offers are removed with their server-side transfer data.

Android offers opt-in periodic notification checks through the operating system's WorkManager. Each run validates the saved server ID before reading the credential, processes only bounded `notification.deliver` events, and acknowledges them after local delivery. When background incoming review is also enabled, the worker validates offer metadata and posts a private notification. Decline can be completed headlessly. Accepting a file schedules a network-constrained worker that revalidates the profile, offer metadata, exact byte count and SHA-256 digest before a background-engine plugin writes it through MediaStore. The event is acknowledged only after MediaStore succeeds; a failed action is retained for a later safe retry. Text, URL and clipboard acceptance still require the foreground interface. Android controls the schedule with a minimum periodic interval of about 15 minutes and may delay one-off work, so this is not advertised as realtime or persistent presence.
...
Android may optionally discover incoming clipboard, text, URL and file offers during the same periodic background check. The background worker validates the server identity and bounded event metadata, displays a private notification without embedding the shared value or filename, and deliberately leaves the event unacknowledged until the requested action succeeds. Accept and Decline actions are recorded by server profile and event ID; processing revalidates the profile and event before any acknowledgement. The optional seamless setting accepts only files whose event carries the server-derived `sameAccount` marker, including from a background check. It never applies to household devices, URLs, text, or clipboard. Notification deduplication stores event identifiers only. Multiple offers are queued independently and remain isolated to the authenticated profile and server-enforced account or household scope.
